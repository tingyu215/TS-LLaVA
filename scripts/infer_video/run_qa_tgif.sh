...
video_dir="BASE_VIDEO_DIR/TGIF_Zero_Shot_QA/all_test"
...
for IDX in $(seq 0 $((CHUNKS-1))); do
  CUDA_VISIBLE_DEVICES=${GPULIST[$IDX]} python3 ./TS-LLaVA/scripts/infer_video/run_inference_video_qa.py \
      --video_dir ${video_dir} \
      --gt_file_question ${GT_QA_DIR}/TGIF_Zero_Shot_QA/test_q.json \
      --gt_file_answers ${GT_QA_DIR}/TGIF_Zero_Shot_QA/test_a.json \
